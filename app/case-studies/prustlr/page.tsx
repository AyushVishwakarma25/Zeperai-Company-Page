...
const reelSources = [
  { kind: "video", src: "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/Discovery%20Motion%20ad-qLV8UA6m0LLt6iGwRwiISUJC0R0Ryr.mp4", label: "Prustlr discovery pack motion ad" },
  { kind: "instagram", src: "https://www.instagram.com/reel/DcijvHWuVkG/embed/", label: "Prustlr Instagram reel" },
  { kind: "website", src: "https://prustlr.com/products/prustlrs-discovery-pack-everyday-protein-oats", label: "Prustlr discovery pack product page" },
]

const adCreatives = [
  { src: "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/Discovery%20Motion%20ad-qLV8UA6m0LLt6iGwRwiISUJC0R0Ryr.mp4", label: "Ad creative / 01" },
  { src: "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/tiramisu%20call-lUuDsqJtnJnmxBxxzCexOPwJx39kUR.webp", label: "Ad creative / 02" },
  { src: "https://hebbkx1anhila5yf.public.blob.vercel-storage.com/sample%202-rZRR0HXkwFVdtSVI3GZzRSVXGWELis.webp", label: "Ad creative / 03" },
]
...
export default function PrustlrCaseStudy() {
  return <main className="min-h-screen overflow-hidden bg-[#f5f5f7] text-[#0A0A0B]">
    ...
    <section className="bg-[#151518] px-6 py-24 text-white md:px-10 md:py-32"><div className="mx-auto max-w-7xl"><SectionLabel number="03" eyebrow="Ad creatives" title="Thumb-stopping creative for the feed." copy="Three 9:16 concepts built to make flavour, format, and product value land in the first frame."/><div className="grid gap-4 md:grid-cols-3">{adCreatives.map((tile) => <article key={tile.label} className="relative aspect-[9/16] overflow-hidden rounded-[1.5rem] bg-[#2b2b30]"><video src={tile.src.endsWith('.mp4') ? tile.src : undefined} poster={tile.src.endsWith('.mp4') ? undefined : tile.src} controls={tile.src.endsWith('.mp4')} muted playsInline className="h-full w-full object-cover" />{!tile.src.endsWith('.mp4') && <img src={tile.src} alt={tile.label} className="h-full w-full object-cover" />}<p className="absolute left-4 top-4 rounded-full bg-black/65 px-3 py-2 text-[10px] font-semibold uppercase tracking-[.18em] text-white backdrop-blur">{tile.label}</p></article>)}</div></div></section>

    <section className="px-6 py-24 md:px-10 md:py-32"><div className="mx-auto max-w-7xl"><SectionLabel number="04" eyebrow="Reel system" title="Short-form stories built to stop the thumb." copy="The video system is ready for embedded reel links. Replace each placeholder with the final hosted video URL when the cuts are ready."/><div className="grid gap-4 md:grid-cols-3">{reelSources.map((reel, index) => <article key={reel.label} className="overflow-hidden rounded-[1.5rem] bg-[#101113] text-white"><div className="relative aspect-[9/16]">{reel.kind === "video" ? <video src={reel.src} controls muted playsInline className="h-full w-full object-cover" /> : <iframe title={reel.label} src={reel.src} className="h-full w-full border-0" allow="autoplay; fullscreen; picture-in-picture" />}</div><div className="p-5"><p className="font-mono text-xs text-[#a8b0ff]">0{index + 1}</p></div></article>)}</div></div></section>
